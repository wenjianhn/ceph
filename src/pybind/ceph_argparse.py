...
def find_cmd_target(childargs):
    ...
    if len(childargs) > 1 and childargs[0] == 'tell':
        name = CephName()
        # CephName.valid() may raise two exceptions, both of which should be
        # handled by the caller:
        #
        #  ArgumentFormat   if target has no '.' in the name or, if target is an
        #                   osd, id is not an integer
        #  ArgumentValid    if target type is not recognized
