...
sub delete_user
{
    my $cmd = "./radosgw-admin user rm --uid=$rgw_user";
...
sub quota_set_max_size {
    my $set_quota = `./radosgw-admin quota set --bucket=$bucketname --max-size=1048576000`; 
    if ($set_quota !~ /./){
...
sub quota_set_max_size_zero {
    run_s3($rgw_user);
    my $set_quota = `./radosgw-admin quota set --bucket=$bucketname --max-size=0`; 
    if ($set_quota !~ /./){
...
sub quota_set_max_objs_zero {
    run_s3($rgw_user);
    my $set_quota = `./radosgw-admin quota set --bucket=$bucketname --max-objects=0`; 
    if ($set_quota !~ /./){
...
sub quota_set_neg_size {
    run_s3($rgw_user);
    my $set_quota = `./radosgw-admin quota set --bucket=$bucketname --max-size=-1`; 
    if ($set_quota !~ /./){
...
sub quota_set_neg_objs {
    run_s3($rgw_user);
    my $set_quota = `./radosgw-admin quota set --bucket=$bucketname --max-objects=-1`; 
    if ($set_quota !~ /./){
...
sub quota_set_user_objs {
    my $set_quota = `./radosgw-admin quota set --uid=$rgw_user --quota-scope=bucket`; 
    my $set_quota1 = `./radosgw-admin quota set --bucket=$bucketname --max-objects=1`; 
    if ($set_quota1 !~ /./){
...
sub quota_set_user_size {
    my $set_quota = `./radosgw-admin quota set --uid=$rgw_user --quota-scope=bucket`; 
    my $set_quota1 = `./radosgw-admin quota set --bucket=$bucketname --max-size=1048576000`; 
    if ($set_quota1 !~ /./){
...
sub quota_enable {
    my $en_quota = `./radosgw-admin quota enable --bucket=$bucketname`; 
    if ($en_quota !~ /./){ 
...
sub quota_disable {
    my $dis_quota = `./radosgw-admin quota disable --bucket=$bucketname`; 
    if ($dis_quota !~ /./){ 
